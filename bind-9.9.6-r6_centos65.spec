#
# spec v0.8 file for package bind-9.9.6-r6
#
...
# allways higher then distro Epoch
%define epoch 65
#
...
Provides:       caching-nameserver = %{epoch}:%{version}-%{release}
Obsoletes:      bind < 9.9.5
Obsoletes:      named < 9.9.5
Obsoletes:      nameserver < 9.9.5
Obsoletes:      dns_daemon < 9.9.5
Conflicts:      caching-nameserver < 31:9.8.3
Conflicts:      caching-nameserver < 31:9.8.3
Requires:       %{NAME}-utils = %{epoch}:%{version}-%{release}
Requires:       bind-license = %{epoch}:%{version}-%{release}
Obsoletes:      dnssec-conf < 1.27-2
Provides:       dnssec-conf = %{epoch}:%{version}-%{release}
...
Patch75: bind-96-libtool2-9.9.3.patch
...
Requires:       bind-license = %{epoch}:%{version}-%{release}
Provides:       bind-devel = %{epoch}:%{version}-%{release}
Provides:       bind8-devel = %{epoch}:%{version}-%{release}
Provides:       bind9-devel = %{epoch}:%{version}-%{release}
Provides:       devel = %{epoch}:%{version}-%{release}
Obsoletes:      bind8-devel < 9.8.5
Obsoletes:      bind9-devel < 9.8.5
Conflicts:      devel  < 75:
...
Provides:       lite-devel = %{epoch}:%{version}-%{release}
Requires:       bind-license = %{epoch}:%{version}-%{release}
Conflicts:      lite-devel < 75:
...
Obsoletes:      bind-libbind-devel < 9.9.3
Provides:       bind-libbind-devel = 9.9.9
Provides:       libs-lite = %{epoch}:%{version}-%{release}
Requires:       bind-license = %{epoch}:%{version}-%{release}
Conflicts:      libs-lite < 75:
...
Group:    Applications/System
BuildArch:      noarch
Conflicts:      bind-license < 75:
Conflicts:      bind9-license < 75:
Conflicts:      named-license < 75:
...
Requires:       bind-license = %{epoch}:%{version}-%{release}
Provides:       pkcs11 = %{epoch}:%{version}-%{release}
...
Requires:       bind-license = %{epoch}:%{version}-%{release}
Provides:       doc = %{epoch}:%{version}-%{release}
...
Group:          Development/Libraries/C and C++
Requires:       bind-license = %{epoch}:%{version}-%{release}
Provides:       libs = %{epoch}:%{version}-%{release}
...
Requires:       %{NAME}-chroot = %{epoch}:%{version}-%{release}
Requires:       bind-license = %{epoch}:%{version}-%{release}
Provides:       lwresd = %{epoch}:%{version}-%{release}
...
Group:          Productivity/Networking/DNS/Utilities
Provides:       bind9-utils = %{epoch}:%{version}-%{release}
Provides:       bindutil = %{epoch}:%{version}-%{release}
Provides:       dns_utils = %{epoch}:%{version}-%{release}
Provides:       utils = %{epoch}:%{version}-%{release}
Obsoletes:      bind-utils < 9.9.5
Obsoletes:      bind9-utils <  9.9.5
Obsoletes:      bindutil < 9.9.5
Requires:       bind-license = %{epoch}:%{version}-%{release}
...
Requires:       bind-license = %{epoch}:%{version}-%{release}
Provides:       chroot  = %{epoch}:%{version}-%{release}
...
install -m 644 %{SOURCE6} ${RPM_BUILD_ROOT}%{_sysconfdir}/portreserve/named
...
mkdir -p sample
tar -xpjf %{SOURCE28} -C sample

test -d  ${RPM_BUILD_ROOT}/etc/named || mkdir ${RPM_BUILD_ROOT}/etc/named

install -m 644 bind.keys ${RPM_BUILD_ROOT}/var/named/etc/named.iscdlv.key
install -m 644 %{SOURCE36} ${RPM_BUILD_ROOT}/var/named/etc/trusted-key.key
...
install -m 644 sample/var/named/named.ca ${RPM_BUILD_ROOT}/var/named/named.ca
install -m 644 sample/var/named/named.empty ${RPM_BUILD_ROOT}/var/named/named.empty
install -m 644 sample/var/named/named.localhost ${RPM_BUILD_ROOT}/var/named/named.localhost
install -m 644 sample/var/named/named.empty ${RPM_BUILD_ROOT}/var/named/named.loopback
#
# Copy default sample config - caching rfc1912
install -m 644 sample/etc/named.rfc1912.zones ${RPM_BUILD_ROOT}/var/named/named.rfc1912.zones
#
# Copy default sample named config - caching rfc1912
install -m 644 sample/etc/named.conf ${RPM_BUILD_ROOT}/etc/named.conf
install -m 644 sample/etc/named.root.key ${RPM_BUILD_ROOT}/var/named/etc/named.root.key
#
#  Copy default sample rndc config
...
%config(noreplace) %attr(-,root,named) /var/named/etc/named.iscdlv.key
%config(noreplace) %attr(-,root,named) /var/named/etc/named.root.key
...
# Hide configuration , don't  override if files exist
%defattr(0640,root,named,0750)
%dir %{_sysconfdir}/named
%dir %{_localstatedir}/named
%config(noreplace) %verify(not link) %{_sysconfdir}/named.conf
%config(noreplace) %verify(not link) /var/named/named.rfc1912.zones
%config(noreplace) %verify(not link) /var/named/named.ca
%config(noreplace) %verify(not link) /var/named/named.localhost
%config(noreplace) %verify(not link) /var/named/named.loopback
%config(noreplace) %verify(not link) /var/named/named.empty
...
%changelog
* Sat Jan 01 2015  - phess@phess.best.vwh.net -r  9.9.6 -r6
- The /etc/portreserve/named file was being overwritten by ldap2zone.1 man
page. The attached patch corrects that issue.

* Wed Nov 19 2014 Horst venzke - support@remsnet.de -r  9.9.6 -r5
- added Epoc > 65, righer then Distro to avoid RPM´s updated by yum.
* Wed Nov 19 2014 Horst venzke - support@remsnet.de -r  9.9.6 -r4
- enabled SDB - DLOPEN
- enabled SDB - LDAP DLZ
- enabled SDB - DBD DLZ
- enabled SDB - FILESYSTEM DLZ
- enabled SDB - GSSTSIG
- deactivated SDB - POSTGRES DLZ
- deactivated SDB - MYSQLDB DLZ
- enabled SPNEGO
- enabled ECDSA --with-ecdsa
- enabled IDN --with-idnlib='-L/usr/lib -R/usr/lib -lidn -lidn2'
- enable-newstats
- enable-exportlib
- enable-fixed-rrset
- enable-rrl
- rewrote sample configs for caching dns  based on /var/named/* ( Source28 )
- enable-filter-aaaa
- removed bind-chroot requirement - this is optional as we what build and use for samba DLZ
  bind-chroot are incompatible with samba4 LDAP DLZ
